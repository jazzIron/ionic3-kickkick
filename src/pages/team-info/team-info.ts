...
export class TeamInfoPage {
  ...
  nextMatch_url: string;
  nextMatchInfoList: any;
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public http: HttpClient) {
    ...
    this.nextMatch_url = environment.getNextMatch;
  ...
  recentHistoryFnc() {
    ...
    this.data.subscribe(data => {
      ...
      console.log(this.getRecentHistory);
    }, (err) => {
      console.log("data Error");
    });
  }
...
/*         data.goalMarginsList.goal4[0].num,
  ...
  matchHistoryFnc(type: any) {
    ...
    nextMatchFnc() {
      ...
      postData.append('teamId', this.teamId);
      postData.append("areaType", "E");
      this.data = this.http.post(this.nextMatch_url, postData);
      this.data.subscribe(data => {

        let infodata = data.nextMatchInfo[0];
        let teamId, teamNm, teamImg = '';

        if(infodata.homeTeamId == this.teamId){
          teamId = infodata.awayTeamId;
          teamNm = infodata.awayShortNm;
          teamImg = infodata.awayTeamImg2;
        }else{
          teamId = infodata.homeTeamId;
          teamNm = infodata.homeShortNm;
          teamImg = infodata.homeTeamImg2;
        }

        this.nextMatchInfoList =[{
          teamId : teamId,
          teamNm : teamNm,
          teamImg : teamImg,
          matchDate : infodata.matchDateKor,
          matchRound: infodata.matchRound
        }];
    ...
    detailMatchInfo(teamId){
      this.navCtrl.push(TeamComparePage, teamId);
    }
